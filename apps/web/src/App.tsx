...
type ProvisionalReasoningSummary = {
  ...
  reasoning: {
    ...
    controls?: { confound: string; experiment: string; rationale: string }[];
...
type Detail = {
  ...
  draft?: {
    ...
    provisional_reasoning?: ProvisionalReasoningSummary;
  };
  packet?: {
    claim: { claim: string };
    sources: Source[];
...
function humanizeServiceError(message: string) {
  if (message.includes("methods must contain")) return "Add a short method context sentence before creating the Run.";
  if (message.includes("at least one reviewed direct source")) return "Source Review is not ready: Codex must mark at least one candidate as direct evidence before freeze.";
  if (message.includes("source roles must be reviewed")) return "Review source roles first. Search results are only candidates until Codex assigns direct/contextual/reject.";
  if (message.includes("binding requires one X column")) return "Confirm which column is the independent axis and which column is the observable.";
  if (message.includes("confirmed binding")) return "Confirm the artifact column binding before freezing this Run.";
  return message;
}
...
async function api<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  if (!response.ok) {
    const body = (await response.json().catch(() => null)) as
      ...
      | null;
    throw new Error(humanizeServiceError(body?.detail?.message ?? "GroundLoop local service is unavailable."));
  }
...
function freezeReadiness(detail: Detail) {
  ...
  const bindingReady = detail.run.workflow !== "generic_v2" || artifacts.length === 0 || artifacts.every((artifact) => bindings.some((binding) => binding.artifact_id === artifact.artifact_id));
  const readyToFreeze = detail.run.state === "DRAFT" && sources.length > 0 && directCount > 0 && !reviewPending && bindingReady;
  const actionLabel = bindingReady ? (sources.length === 0 ? "IMPORT SOURCES FIRST" : directCount === 0 || reviewPending ? "REVIEW SOURCES FIRST" : "FREEZE EVIDENCE") : "CONFIRM BINDING FIRST";
  return { sources, review, adjudications, directCount, reviewPending, bindingReady, readyToFreeze, actionLabel };
}
...
function artifactColumnIdText(profiles: GenericProfile[], artifacts: Artifact[] = [], bindings: ArtifactBinding[] = []) {
...
function genericBriefContext(detail: Detail) {
  ...
  return [
    ...
    ...artifactColumnIdText(profiles, artifacts, bindings).split("\n").map((line) => line.startsWith("  ") ? `  - ${line.trim()}` : `- ${line}`),
  ];
}
...
function Landing({
  ...
  const methodReady = methods.trim().length >= 20;
  const csvReady = datasetCsv.trim().split(/\r?\n/).length >= 2 && datasetCsv.includes(",");
  const canCreate = claimReady && methodReady && csvReady && !busy;
    ...
    <main className="landing-layout">
      ...
      <section className="landing-hero">
        ...
        <div className="entry-panel panel-linework">
          ...
          <div className="entry-checklist" aria-label="Run setup checklist">
            <StepPill ready={claimReady} label="Claim" />
            <StepPill ready={methodReady} label="Method context" />
            <StepPill ready={csvReady} label="CSV shape" />
          </div>
          ...
          <p className="field-help">Write the thing you want to test, not the conclusion you want GroundLoop to approve.</p>
          ...
          <p className="field-help">Include what was measured, what was held fixed, and what control is missing if you already know it.</p>
...
function StepPill({ ready, label }: { ready: boolean; label: string }) {
  return <span className={`step-pill ${ready ? "ready" : ""}`}>{ready ? <Check size={12} /> : <Activity size={12} />}{label}</span>;
}
...
function MapScreen({ detail, onNotice, onFreeze, onRefresh }: { detail: Detail; onNotice: (notice: string) => void; onFreeze: () => Promise<void>; onRefresh: () => void }) {
  ...
  const readiness = freezeReadiness(detail);
...
function BindingPanel({ detail, profile, artifact, onNotice, onRefresh }: { detail: Detail; profile: GenericProfile; artifact?: Artifact; onNotice: (notice: string) => void; onRefresh: () => void }) {
  ...
  const copyIds = async () => {
    try {
      await navigator.clipboard.writeText(artifactColumnIdText([profile], artifact ? [artifact] : []));
      onNotice("Artifact and column IDs copied. Use these exact IDs in GroundLoop MCP calls.");
    } catch {
      onNotice("Clipboard access was unavailable. The artifact and column IDs remain visible here.");
    }
  };
  return <section className="binding-panel"><div><p className="kicker">RESEARCHER CONFIRMATION / DATA BINDING</p><h2>{artifact?.label ?? profile.artifact_id}: columns are not scientific roles until you confirm them.</h2><div className="id-strip"><span>{profile.artifact_id}</span><button type="button" onClick={() => void copyIds()}><Clipboard size={12} /> COPY IDS</button></div>{codexProposed ? <p><strong>CODEX PROPOSAL / {proposal.replaceAll("_", " ")}</strong> · {routing?.reasons[0] ?? "Recorded from the Run context."} Confirm the primary axis and observable before the packet can freeze. This proposal does not constrain Codex's later signatures or controls.</p> : <p><strong>CODEX MODALITY PENDING / HEADER SIGNAL ONLY</strong> · GroundLoop's header signal is advisory only. Confirm this binding as generic, or ask Codex to read the method and literature before it records optional capability metadata.</p>}</div><div className="binding-controls"><label><span>X / INDEPENDENT</span><select value={xColumnId} onChange={(event) => setXColumnId(event.target.value)}>{profile.columns.map((column) => <option value={column.column_id} key={column.column_id}>{column.column_id} · {column.name} · {column.unit.value ?? "unit unknown"}</option>)}</select></label><label><span>Y / OBSERVABLE</span><select value={yColumnId} onChange={(event) => setYColumnId(event.target.value)}>{numeric.map((column) => <option value={column.column_id} key={column.column_id}>{column.column_id} · {column.name} · {column.unit.value ?? "unit unknown"}</option>)}</select></label><button type="button" className="primary-button" onClick={() => void confirm()}>CONFIRM {codexProposed ? "PROPOSED" : "GENERIC"} BINDING <Check size={15} /></button></div></section>;
}
...
function GenericEvidenceDeck({ artifacts, profiles, bindings, evidence, method, onNotice }: { artifacts: Artifact[]; profiles: GenericProfile[]; bindings: ArtifactBinding[]; evidence: Detail["data_evidence"]; method: string; onNotice: (notice: string) => void }) {
  ...
  const copyIds = async () => {
    try {
      await navigator.clipboard.writeText(artifactColumnIdText(profiles, artifacts, bindings));
      onNotice("Artifact and column IDs copied for Codex MCP calls.");
    } catch {
      onNotice("Clipboard access was unavailable. The artifact and column IDs remain visible here.");
    }
  };
...
function SourcesScreen({ detail, onFreeze, onView }: { detail: Detail; onFreeze: () => Promise<void>; onView: (view: View) => void }) {
  const { sources, adjudications, directCount, reviewPending, bindingReady, readyToFreeze } = freezeReadiness(detail);
  const sourceStepReady = sources.length > 0;
  const reviewStepReady = directCount > 0 && !reviewPending;
  return <div className="secondary-page"><div className="secondary-heading"><div><p className="kicker">EVIDENCE ROLE REVIEW / SOURCE LEDGER</p><h1>Every source earns its place.</h1><p>Search results start as candidates. Codex must review the excerpt and assign a role before a source can support the decision.</p></div><div className="review-count"><strong>{directCount.toString().padStart(2, "0")}</strong><span>DIRECT<br />UNITS</span></div></div><div className="review-steps" aria-label="Freeze readiness checklist"><div className={sourceStepReady ? "ready" : ""}><StepPill ready={sourceStepReady} label="Candidates imported" /><p>{sourceStepReady ? `${sources.length} bounded candidate${sources.length === 1 ? "" : "s"} in this Run.` : "Ask Codex to import bounded literature candidates first."}</p></div><div className={reviewStepReady ? "ready" : ""}><StepPill ready={reviewStepReady} label="Direct source reviewed" /><p>{reviewStepReady ? "At least one direct source has a semantic role." : "Codex must mark direct/contextual/reject and assign roles."}</p></div><div className={bindingReady ? "ready" : ""}><StepPill ready={bindingReady} label="Data binding confirmed" /><p>{bindingReady ? "The measurement artifact has confirmed column roles." : "Return to Convergence Map and confirm X/Y column binding."}</p></div></div><div className="review-toolbar"><span>{sources.length} candidates · {directCount} direct · {detail.run.state === "DRAFT" ? "UNFROZEN" : "BOUNDARY FROZEN"}</span>{reviewPending && <span className="tiny-note">SOURCE REVIEW STALE · Codex must review the current candidate set again.</span>}<span className="tiny-note"><LockKeyhole size={13} /> Source content is untrusted input.</span></div><div className="source-ledger"><div className="ledger-head"><span>SOURCE / BOUNDED EXCERPT</span><span>SEMANTIC REVIEW</span><span>EVIDENCE ROLE</span><span>PROVENANCE</span></div>{sources.length === 0 ? <div className="ledger-empty"><Beaker size={19} /><span>No literature candidates yet. Open the Run brief, paste it into Codex, and ask it to import bounded source candidates.</span></div> : sources.map((source) => { const adjudication = adjudications.find((item) => item.source_id === source.id); const state = adjudication?.verdict ?? "unreviewed"; return <article className="source-row" key={source.id}><div className="source-main"><span className="source-id">{source.id}</span><h3>{source.title}</h3><p>{source.untrusted_content}</p><a href={source.url_or_doi} target="_blank" rel="noreferrer">{source.locator.section ?? source.url_or_doi}<ArrowRight size={12} /></a></div><div><span className={`source-state ${state}`}>{state.toUpperCase()}</span><p className="source-rationale">{adjudication?.rationale ?? "Awaiting Codex semantic review."}</p></div><div><span className={`role-chip ${adjudication?.role ? "assigned" : ""}`}>{sourceRoleLabel(adjudication?.role)}</span><p className="source-rationale">{adjudication?.role === "theory_basis" ? "What must be true if the mechanism is right." : adjudication?.role === "method_limit" ? "What this measurement cannot distinguish." : adjudication?.role === "discriminating_control" ? "What separates the competing explanations." : "One role is required before this source becomes decision evidence."}</p></div><div className="source-provenance"><span>{source.retrieval_provider.toUpperCase()} / {source.publication_status.replaceAll("_", " ").toUpperCase()}</span><span>{source.year} · {source.authors[0]}</span><span className="hash-line">LOCATOR · {source.locator.section ?? "ABSTRACT"}</span><span className="hash-line">EXCERPT SHA · {(source.content_sha256 ?? "not recorded").slice(0, 16)}</span>{source.search_query && <span className="hash-line">QUERY · {source.search_query}</span>}</div></article>; })}</div><div className={`freeze-callout ${readyToFreeze ? "" : "not-ready"}`}><div>{readyToFreeze ? <LockKeyhole size={18} /> : <Clipboard size={18} />}<div><strong>{detail.run.state === "DRAFT" ? (readyToFreeze ? "Evidence freeze belongs to the researcher." : "Not ready to freeze yet.") : "Evidence boundary is frozen."}</strong><span>{detail.run.state === "DRAFT" ? (bindingReady ? sourceNextAction(sources, directCount, reviewPending) : "Next: confirm the measurement artifact binding before this packet can freeze.") : "Changing the claim or method now requires a Fork Run."}</span></div></div>{detail.run.state === "DRAFT" ? (readyToFreeze ? <button type="button" className="freeze-action" onClick={() => void onFreeze()}><LockKeyhole size={14} /> FREEZE EVIDENCE</button> : <button type="button" className="freeze-action" onClick={() => onView(bindingReady ? "audit" : "map")}><Clipboard size={14} /> {bindingReady ? "OPEN RUN BRIEF" : "CONFIRM BINDING"}</button>) : <span className="freeze-stamp">IMMUTABLE PACKET</span>}</div></div>;
}
