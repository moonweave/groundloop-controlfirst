...
7. Codex requests allowlisted operations such as `band_comparison`, `local_peak`, `group_comparison`, `argmax`, `endpoint_delta`, `grouped_extrema`, `hysteresis_window`, `power_law_fit`, `linear_fit`, `correlation`, or `monotonicity` against explicit artifact IDs. GroundLoop returns the only valid data-evidence IDs and calculated facts.
