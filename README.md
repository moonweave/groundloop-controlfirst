...
explicit local-peak or extrema fact but conservatively leaves microscopic
...
- `create_generic_run`, `add_measurement_artifact`, `inspect_measurement_artifacts`, `inspect_dataset_profile`, `propose_measurement_modality`, `record_measurement_modality`, `record_provisional_reasoning`, `set_dataset_binding`, `set_artifact_binding` — v2 generic intake, Codex-authored advisory routing, draft-only exploratory reasoning, researcher-confirmed measurement roles, and optional non-constraining capability pack metadata;
